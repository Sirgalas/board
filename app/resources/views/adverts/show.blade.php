...
@section('content')
    ...
    @can ('manage-adverts')
        <div class="d-flex flex-row mb-3">
            ...
            @if ($advert->isOnModeration())
                {{Form::open(["route"=>['admin.adverts.adverts.moderate', $advert],'class'=>'mr-1','action'=>'post'])}}
                    {{Form::token()}}
                    {{Form::submit('Отметить модерацию',['class'=>"btn btn-success"])}}
                {{Form::close()}}
            @endif
            ...
            @endif
            {{Form::open(['route'=>['admin.adverts.adverts.destroy', $advert],'class'=>'mr-1','method'=>'delete'])}}
                {{Form::token()}}
    ...
    @can ('manage-own-advert', $advert)
        <div class="d-flex flex-row mb-3">
            ...
            @if ($advert->isDraft())
                {{Form::open(['route'=>(['cabinet.adverts.send', 'advert'=>$advert]),"class"=>"mr-1"])}}
                    {{Form::token()}}
                    {{Form::submit('Опубликовать',["class"=>"btn btn-success"])}}
                {{Form::close()}}
            @endif
            @if ($advert->isActive())
                {{Form::open(['route'=>(['cabinet.adverts.close', 'advert'=>$advert]),"class"=>"mr-1",'method'=>'delete'])}}
                    {{Form::token()}}
                    {{Form::submit('Закрыть',["class"=>"btn btn-success"])}}
                {{Form::close()}}
            @endif
            {{Form::open(['route'=>(['cabinet.adverts.close', 'advert'=>$advert]),"class"=>"mr-1",'method'=>'delete'])}}
                {{Form::token()}}
