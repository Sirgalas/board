...
@section('content')
    ...
    <div class="d-flex flex-row mb-3">
        @if ($ticket->canBeRemoved())
            {{Form::open(['route'=>['cabinet.tickets.destroy', $ticket],'class'=>'mr-1'])}}
                @method('DELETE')
    ...
    @if ($ticket->allowsMessages())
        {{Form::open(['route'=>['cabinet.tickets.message', $ticket]])}}
            @csrf

            <div class="form-group">
                {{Form::textarea('message',old('message'),['class'=>$errors->has('message') ?'form-control is-invalid':'form-control',"rows"=>"3", "required"=>true])}}
